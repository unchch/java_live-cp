...
import com.bh.live.dao.NumberSingleDoubleMissDao;
import com.bh.live.model.inform.Omission;
import com.bh.live.service.NumberSingleDoubleMissService;
 ...
 */
@Service
public class NumberSingleDoubleMissImpl implements NumberSingleDoubleMissService {
    @Resource
    private NumberSingleDoubleMissDao singleDoubleMissDao;
    ...
    public List<Omission> getCurrentOmission(Integer number, int varietyType) {
        ...
        if (number == null) {
            ...
            if (varietyType == 0 || varietyType == 1) {
                ...
                currentOmissions = singleDoubleMissDao.currentOmission(varietyType, fieldCondition);
            ...
            String fieldCondition = "if(" + NumberHandlerHelper.getBallFiled(number).get(0) + "%2=0,'双','单') singleDouble";
            List<Omission> currentOmissions = singleDoubleMissDao.currentOmission(varietyType, fieldCondition);
    ...
    private List<Omission> getAllOmissions(int varietyType, Integer number, String timeCondition, String dateType, String flag) {
        ...
        if (number == null) {
            ...
            if (varietyType == 0 || varietyType == 1) {
                ...
                currentOmissions = singleDoubleMissDao.getDateOmission(varietyType, sql, null);
            ...
            fieldStr = "if(" + NumberHandlerHelper.getBallFiled(number).get(0) + "%2=0,'双','单') singleDouble";
            String sql = "(SELECT expect," + fieldStr + " FROM t_variety_current WHERE variety_type = #{varietyType} " +
                    timeCondition + " order by open_time desc)";
            List<Omission> todayOmission = singleDoubleMissDao.getDateOmission(varietyType, sql, null);
            if (flag.equals("todayAppear")) {
