...
import com.bh.live.dao.NumDragonTigerMissDao;
import com.bh.live.model.inform.Omission;
import com.bh.live.service.NumberDragonTigerMissService;
 ...
 */
@Service
public class NumberDragonTigerMissImpl implements NumberDragonTigerMissService {
    @Resource
    private NumDragonTigerMissDao dragonTigerMissDao;
    ...
    public List<Omission> getCurrentOmission(Integer number, int varietyType) {
        ...
        if (number == null) {
            ...
            if (varietyType == 0 || varietyType == 1) {
                for (int i = 1; i <= 5; i++) {
                    String ifStr = "if(" + NumberHandlerHelper.getDragonTigerFiled(i).get(0) + "='龙','龙','虎') " + NumberHandlerHelper.getDragonTigerFiled(i).get(1) + ",";
                    stringBuffer.append(ifStr);
                }
                fieldCondition = stringBuffer.substring(0, stringBuffer.length() - 1);
                currentOmissions = dragonTigerMissDao.currentOmission(varietyType, fieldCondition);
                List<Omission> currentOmission = AllOmissionHelper.getDragonTigerCurrentOmission(varietyType, currentOmissions, currentOmissions.get(0));
            ...
            String fieldCondition = "if(" + NumberHandlerHelper.getDragonTigerFiled(number).get(0) + "='龙','龙','虎')   singleDouble";
            List<Omission> currentOmissions = dragonTigerMissDao.currentOmission(varietyType, fieldCondition);
    ...
    private List<Omission> getAllOmissions(int varietyType, Integer number, String timeCondition, String dateType, String flag) {
        ...
        if (number == null) {
            ...
            if (varietyType == 0 || varietyType == 1) {
                ...
                currentOmissions = dragonTigerMissDao.getDateOmission(varietyType, sql, null);
                
                if (flag.equals("todayAppear")) {
                    return currentOmissions;
                }
                List<Omission> currentOmission = AllOmissionHelper.getDragonTigerDayOmiss(varietyType, currentOmissions, dateType);
            ...
            fieldStr = "if(" + NumberHandlerHelper.getDragonTigerFiled(number).get(0) + "='龙','龙','虎')  singleDouble";
            String sql = "(SELECT expect," + fieldStr + " FROM t_variety_current WHERE variety_type = #{varietyType} " +
                    timeCondition + " order by open_time desc)";
            List<Omission> todayOmission = dragonTigerMissDao.getDateOmission(varietyType, sql, null);
            if (flag.equals("todayAppear")) {
            ...
            return PackageSingleDoubleOmission(number, varietyType, twoFaceMaxOmission, dateType);
        }
    }
