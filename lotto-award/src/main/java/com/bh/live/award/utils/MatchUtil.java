package com.bh.live.award.utils;
...
public class MatchUtil {
	 ...
	 * @Desc 对double数据进行相乘并取精度.
	 */
	public static double mul(double d1, double d2, int scale, int roundingMode) {
		try {
			BigDecimal bd1 = new BigDecimal(Double.toString(d1));
			BigDecimal bd2 = new BigDecimal(Double.toString(d2));
			return bd1.multiply(bd2).setScale(scale, roundingMode).doubleValue();
		} catch (Exception e) {
			return 0;
		}
	}
